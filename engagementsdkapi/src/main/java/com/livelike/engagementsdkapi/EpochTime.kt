...
class EpochTime(val timeSinceEpochInMs: Long) : Comparable<EpochTime> {
    ...
    operator fun plus(et: EpochTime) = EpochTime(timeSinceEpochInMs + et.timeSinceEpochInMs)
    operator fun plus(timeStamp: Long) = EpochTime(timeSinceEpochInMs + timeStamp)
}
