...
class MessageSwipeController(
    ...
    override fun getMovementFlags(
        ...
        imageDrawable = ContextCompat.getDrawable(context, R.drawable.ic_chat_keyboard)!!
        shareRound = ContextCompat.getDrawable(context, R.drawable.chat_ic_flag)!!
    ...
    override fun onChildDraw(
        ...
        if (mView.translationX < AndroidResource.pxToDp(130) || dX < this.dX) {
    ...
    private fun setTouchListener(recyclerView: RecyclerView, viewHolder: RecyclerView.ViewHolder) {
        recyclerView.setOnTouchListener { _, event ->
            ...
            if (swipeBack) {
                if (abs(mView.translationX) >= AndroidResource.pxToDp(100)) {
    ...
    private fun drawReplyButton(canvas: Canvas) {
        ...
        val showing = translationX >= AndroidResource.pxToDp(30)
        if (showing) {
        ...
        if (startTracking) {
            if (!isVibrate && mView.translationX >= AndroidResource.pxToDp(100)) {
        ...
        val x: Int = if (mView.translationX > AndroidResource.pxToDp(130)) {
            AndroidResource.pxToDp(130) / 2
        } else {
        ...
        shareRound.colorFilter =
            PorterDuffColorFilter(
                ContextCompat.getColor(context, R.color.livelike_alert_background_color),
                PorterDuff.Mode.MULTIPLY
            )

        shareRound.setBounds(
            (x - AndroidResource.pxToDp(18) * scale).toInt(),
            (y - AndroidResource.pxToDp(18) * scale).toInt(),
            (x + AndroidResource.pxToDp(18) * scale).toInt(),
            (y + AndroidResource.pxToDp(18) * scale).toInt()
        )
        shareRound.draw(canvas)
        imageDrawable.setBounds(
            (x - AndroidResource.pxToDp(12) * scale).toInt(),
            (y - AndroidResource.pxToDp(11) * scale).toInt(),
            (x + AndroidResource.pxToDp(12) * scale).toInt(),
            (y + AndroidResource.pxToDp(10) * scale).toInt()
        )
