    ...
    private fun init(centerX: Float, centerY: Float) {
        ...
        if (showThumb) {
            val thumbDrawable = if (thumbDrawableId != 0) ContextCompat.getDrawable(
                context,
                thumbDrawableId
            )!! else ThumbDrawable(thumbColor)
            thumbEntity =
                ThumbEntity(centerPosition, progress, startAngle, thumbRadius, thumbDrawable)
        }
    }

    fun setThumbImage(url: String) {
        println("GaugeSeekBar.setThumbImage->$url")
        Glide.with(this@GaugeSeekBar)
            .asBitmap()
            .load(url)
            .into(object : CustomTarget<Bitmap>() {
                override fun onResourceReady(
                    ...
                    println("GaugeSeekBar.onResourceReady")
                    val d = BitmapDrawable(Resources.getSystem(), resource)
                    d.let {
                        val centerPosition = PointF(measuredWidth / 2f, measuredHeight / 2f)
                        thumbEntity =
                            ThumbEntity(
                                centerPosition,
                                progress,
